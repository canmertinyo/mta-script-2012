...
addEventHandler ( "onClientResourceStart", gResourceRoot, function(name)
		...
		bindKey ( "", "down", workOutRamps, 5 )
		bindKey ( "", "down", workOutRamps, 6 )
	...
	if ( playerVehicle ) then
		...
		if ( mode == "1" ) then
			if ( getKeyState("lshift") ) then
				...
				local ramp2X, ramp2Y = ramp1X+5.5, ramp1Y
				local ramp2Z = ramp1Z 
				local ramp3X, ramp3Y = ramp1X, ramp1Y+5.5 
				local ramp3Z = ramp1Z 
				local ramp4X, ramp4Y = ramp1X-5.5, ramp1Y 
				local ramp4Z = ramp1Z 
				ramp1Y = ramp1Y - 5.5
